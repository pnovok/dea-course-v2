...
df = spark.sql('SELECT * FROM json.`/Volumes/dbdemos_pavel/landing/operational_data/customers`')
display(df)
...
df = spark.sql('''CREATE OR REPLACE TEMP VIEW tv_customers 
                  AS 
                  SELECT * FROM json.`/Volumes/dbdemos_pavel/landing/operational_data/customers`''')
...
df = spark.read.format('json').load('/Volumes/dbdemos_pavel/landing/operational_data/customers')
display(df)
...
df = spark.table('dbdemos_pavel.bronze.v_addresses')
display(df)
